...
class TelegramQuizBot:
    def __init__(self, quiz_manager, db_manager: DatabaseManager | None = None):
        ...
        self.championship_sessions = {}
        logger.info("TelegramQuizBot initialized with performance optimizations: user cache, leaderboard cache, championship mode")
...
🎉 Let's make this group amazing together!"""
    ...
    async def initialize(self, token: str):
        ...
        try:
            ...
            self.application.add_handler(CommandHandler("championship", self.championship))
            ...
            # Add championship callback handler
            self.application.add_handler(CallbackQueryHandler(
                self.handle_championship_callback,
                pattern="^champ_"
            ))
...
/championship - 🏆 Championship Mode (PM only)
    _Test your skills in a 20-question championship!_
    _2 rounds, 10 questions each - prove you're the best!_
...
• Wrong Answers: {wrong_answers}"""
    ...
    async def championship(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """Handle /championship command - Start a 20-question championship mode"""
        if not update.message:
            return
        if not update.effective_user:
            return
        ...
        try:
            ...
            logger.info(f"📥 /championship command received from user {user.id} in chat {chat.id}")
            ...
            if chat.type != 'private':
                ...
                group_msg = f"""🏆 **Championship Mode**
━━━━━━━━━━━━━━━━━━━━━━━

Championship Mode is only available in **private chat**!

👉 Click here to start: @{bot_username}

💡 Why private chat?
• Better focus and experience
• No spam in groups
• Track your personal progress
• Compete on global leaderboard"""
                
                reply_msg = await update.message.reply_text(group_msg, parse_mode=ParseMode.MARKDOWN)
            ...
            if user.id in self.championship_sessions:
                await update.message.reply_text(
                    "⚠️ You already have an active championship!\n\n"
                    "Use /cancel to exit your current championship and start a new one."
                )
                return
            
            all_questions = self.quiz_manager.get_all_questions()
            if len(all_questions) < 20:
                await update.message.reply_text(
                    f"❌ Not enough questions available!\n\n"
                    f"Championship requires 20 questions, but only {len(all_questions)} are available.\n\n"
                    f"Please contact the administrator to add more questions."
                )
                return
            
            loading_msg = await update.message.reply_text("🏆 Starting Championship Mode...\n⏳ Preparing 20 questions...")
            
            championship_questions = random.sample(all_questions, 20)
            
            session = {
                'user_id': user.id,
                'chat_id': chat.id,
                'questions': championship_questions,
                'current_index': 0,
                'answers': [None] * 20,
                'score': 0,
                'round': 1,
            ...
            self.championship_sessions[user.id] = session
            
            await loading_msg.delete()
            
            await self._show_championship_question(user.id, context)
            
            response_time = int((time.time() - start_time) * 1000)
            logger.info(f"/championship started for user {user.id} in {response_time}ms")
            
            self.db.log_performance_metric(
                metric_type='response_time',
                metric_name='/championship',
                value=response_time,
                unit='ms'
            )
            ...
            logger.error(f"Error in championship: {str(e)}\n{traceback.format_exc()}")
            await update.message.reply_text("❌ Error starting championship. Please try again.")
    
    async def _show_championship_question(self, user_id: int, context: ContextTypes.DEFAULT_TYPE):
        """Display current championship question with navigation buttons"""
        try:
            session = self.championship_sessions.get(user_id)
            if not session:
                return
            ...
            question_in_round = (current_idx % 10) + 1
            
            question_text = question['question'].strip()
            if question_text.startswith('/addquiz'):
                question_text = question_text[len('/addquiz'):].strip()
            
            message_text = f"""🏆 **Championship Mode**
━━━━━━━━━━━━━━━━━━━━━━━
**Round {current_round}/2** | **Question {question_in_round}/10**
...
❓ {question_text}"""
            ...
            for i in range(0, 4, 2):
                ...
                for j in range(2):
                    if i + j < 4:
                        label = option_labels[i + j]
                        option_text = options[i + j][:20] + "..." if len(options[i + j]) > 20 else options[i + j]
                        previous_answer = session['answers'][current_idx]
                        
                        if previous_answer == i + j:
                            button_text = f"✓ {label}. {option_text}"
                        else:
                            button_text = f"{label}. {option_text}"
                        
                        row.append(InlineKeyboardButton(
                            button_text,
                            callback_data=f"champ_answer_{i + j}"
                        ))
                keyboard.append(row)
            
            nav_row = []
            if current_idx > 0:
                nav_row.append(InlineKeyboardButton("◀️ Back", callback_data="champ_back"))
            else:
                nav_row.append(InlineKeyboardButton("◀️ Back", callback_data="champ_noop"))
            
            if current_idx == 19:
                nav_row.append(InlineKeyboardButton("✅ Submit", callback_data="champ_submit"))
            else:
                nav_row.append(InlineKeyboardButton("Next ▶️", callback_data="champ_next"))
            
            keyboard.append(nav_row)
            keyboard.append([InlineKeyboardButton("❌ Exit Championship", callback_data="champ_exit")])
            ...
            logger.error(f"Error showing championship question: {str(e)}\n{traceback.format_exc()}")
    
    async def handle_championship_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle all championship callback queries"""
        ...
        try:
            ...
            if not session:
                await query.edit_message_text(
                    "❌ Championship session expired or not found.\n\n"
                    "Use /championship to start a new championship!"
                )
                return
            
            elapsed_time = (datetime.now() - session['start_time']).total_seconds()
            if elapsed_time > 1800:
                await self._championship_timeout(user_id, context)
                return
            
            if query.data.startswith("champ_answer_"):
                answer_idx = int(query.data.split("_")[2])
                current_idx = session['current_index']
                question = session['questions'][current_idx]
                correct_answer = question['correct_answer']
                
                session['answers'][current_idx] = answer_idx
                ...
                if current_idx == 9:
                    round_score = sum(1 for i in range(10) if session['answers'][i] == session['questions'][i]['correct_answer'])
                    await query.edit_message_text(
                        f"🎉 **Round 1 Complete!**\n\n"
                        f"Score: {round_score}/10 {'✅' if round_score >= 5 else '📊'}\n\n"
                        f"Starting Round 2...",
                        parse_mode=ParseMode.MARKDOWN
                    )
                    session['round'] = 2
                ...
                if session['current_index'] >= 20:
                    await self._show_championship_results(user_id, context)
                else:
                    await self._show_championship_question(user_id, context)
                
            elif query.data == "champ_next":
                session['current_index'] += 1
            ...
            elif query.data == "champ_submit":
                await self._show_championship_results(user_id, context)
                
            elif query.data == "champ_exit":
                await query.edit_message_text(
                    "❌ **Championship Exited**\n\n"
                    "Your progress has been discarded.\n\n"
                    "Use /championship to start a new championship anytime!",
                    parse_mode=ParseMode.MARKDOWN
                )
                if user_id in self.championship_sessions:
                    del self.championship_sessions[user_id]
                
            elif query.data == "champ_new":
                if user_id in self.championship_sessions:
                    del self.championship_sessions[user_id]
                ...
                self.championship_sessions[user_id] = session
                await self._show_championship_question(user_id, context)
                
            elif query.data == "champ_noop":
                await query.answer("You're already at the first question!")
                
        except Exception as e:
            logger.error(f"Error in championship callback: {str(e)}\n{traceback.format_exc()}")
            await query.edit_message_text("❌ Error processing championship. Please try /championship again.")
    
    async def _show_championship_results(self, user_id: int, context: ContextTypes.DEFAULT_TYPE):
        """Display championship results with stats and rank"""
        try:
            session = self.championship_sessions.get(user_id)
            if not session:
                return
            
            round1_score = sum(1 for i in range(10) if session['answers'][i] == session['questions'][i]['correct_answer'])
            round2_score = sum(1 for i in range(10, 20) if session['answers'][i] == session['questions'][i]['correct_answer'])
            total_score = round1_score + round2_score
            accuracy = (total_score / 20) * 100
            
            for i, question in enumerate(session['questions']):
                if session['answers'][i] is not None:
                    is_correct = session['answers'][i] == question['correct_answer']
                    question_id = question.get('id')
                    ...
                    activity_date = datetime.now().strftime('%Y-%m-%d')
                    self.db.update_user_score(user_id, is_correct, activity_date)
            ...
            results_text = f"""🏆 **Championship Complete!**
━━━━━━━━━━━━━━━━━━━━━━━━━━━━

**Round 1:** {round1_score}/10 {'✅' if round1_score >= 5 else '📊'}
**Round 2:** {round2_score}/10 {'✅' if round2_score >= 5 else '📊'}
━━━━━━━━━━━━━━━━━━━━━
...
{'🌟 Outstanding performance!' if total_score >= 18 else '🎉 Great job!' if total_score >= 15 else '💪 Keep practicing!' if total_score >= 10 else '📚 Try again!'}"""
            
            keyboard = [
                [
                    InlineKeyboardButton("🎯 New Championship", callback_data="champ_new"),
                    InlineKeyboardButton("📊 My Stats", callback_data="quiz_my_stats")
                ],
                [
                    InlineKeyboardButton("🏆 Leaderboard", callback_data="quiz_leaderboard")
            ...
            logger.info(f"Championship completed for user {user_id}: {total_score}/20")
            
        except Exception as e:
            logger.error(f"Error showing championship results: {str(e)}\n{traceback.format_exc()}")
    
    async def _championship_timeout(self, user_id: int, context: ContextTypes.DEFAULT_TYPE):
        """Handle championship timeout (30 minutes)"""
        try:
            session = self.championship_sessions.get(user_id)
            if not session:
                return
            
            answered_count = sum(1 for ans in session['answers'] if ans is not None)
            
            timeout_text = f"""⏰ **Championship Timeout**
━━━━━━━━━━━━━━━━━━━━━━━

Your championship session has timed out after 30 minutes.

📊 Progress: {answered_count}/20 questions answered
...
Use /championship to start a new championship!"""
            ...
            logger.error(f"Error in championship timeout: {str(e)}\n{traceback.format_exc()}")
    
    async def cancel_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """Handle /cancel command - Exit championship mode"""
        if not update.message:
            return
        if not update.effective_user:
            return
        ...
        try:
            ...
            if user_id in self.championship_sessions:
                ...
                logger.info(f"Championship cancelled for user {user_id}")
            else:
                await update.message.reply_text(
                    "ℹ️ You don't have an active championship.\n\n"
                    "Use /championship to start one!"
                )
        except Exception as e:
            logger.error(f"Error in cancel command: {str(e)}\n{traceback.format_exc()}")
            await update.message.reply_text("❌ Error cancelling championship.")
